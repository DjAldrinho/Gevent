    ...
    if (isset($plantillaPersonal)) {
        Mail::send('emails.template',
            ...
            function ($message) use ($usuario) {
                $message->to('aldray@unisinucartagena.edu.co')
...
function enviarCorreoDiario($usuarios, $dia)
{
    ...
    if (isset($plantillaPersonal)) {
        ...
        $mensaje = "<center><table id='mensual'style='border-collapse:unset;width:60%' border='0'>";
        $mensaje .= "<tr>";
        $mensaje .= "<th>¿Quien cumple hoy?</th>";
        $mensaje .= "</tr>";
        foreach ($usuarios as $usuario) {
            $mensaje .= "<tr><td align='center'>" . $usuario->getNombreCompleto() . "</td></tr>";
        }
